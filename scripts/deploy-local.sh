...
if docker compose -f docker-compose.yml -f docker-compose.localdb.yml logs app 2>&1 | grep -qi "PostgreSQL unavailable"; then
  echo "Error: app fell back to local SQLite — check DB_NAME/DB_USER/DB_PASSWORD/DB_HOST/DB_PORT / DB_SSL_MODE for the bundled Postgres service." >&2
  exit 1
fi
