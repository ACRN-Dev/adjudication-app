...
function Health(){
  ...
  const doResetFull=async()=>{
    if(!confirm('Reset & re-seed complete demo environment?\n\nThis will safely purge all test submissions, refresh demo accounts with password ACRN@2026, and seed 12 baseline cases across all workflow stages (concordant, discordant, 3-way divergent, in-review).')) return;
    const reason=prompt('Enter reason for the full demo reset (required for audit trail):', 'Restoring standard baseline demonstration environment');
    if(!reason?.trim()) return;
    setBusy(true);
    setMsg('');
    try{
      const r=await fetch('/api/admin/demo/reset-environment',{
        method:'POST',
        credentials:'include',
  ...
  const doResetScenarios=async()=>{
    if(!confirm('Clear CSV/imported demo files and assigned adjudications for test scenarios only?\n\nPortal users, sessions, admin fixtures and unrelated records will be left intact.')) return;
    const reason=prompt('Enter reason for the scoped demo reset (required for audit trail):', 'Clearing custom test scenarios');
    if(!reason?.trim()) return;
    setBusy(true);
    setMsg('');
    try{
      const r=await fetch('/api/admin/demo/reset-all',{
        method:'POST',
        credentials:'include',
        headers:{'Content-Type':'application/json'},
        body:JSON.stringify({reason:reason.trim()})
  ...
  return <Page title="Environment and Health" desc="Platform health, database status and demonstration environment controls.">
    ...
    <div className="a-grid2">
      ...
      <section className="a-panel">
        <h2>Demo Environment Reset Controls</h2>
        ...
        <div style={{display:'flex',flexDirection:'column',gap:'10px'}}>
          <button onClick={doResetFull} disabled={busy} style={{background:'#b91c1c',color:'#fff',border:'none',borderRadius:'6px',padding:'10px 18px',fontWeight:700,fontSize:'13px',cursor:busy?'wait':'pointer',opacity:busy?0.7:1,display:'flex',alignItems:'center',gap:'8px'}}>
            <Icon.RotateCcw size={15}/>{busy?'Resetting…':'Reset & Re-seed Full Demo Environment'}
          </button>
          <button onClick={doResetScenarios} disabled={busy} style={{background:'#f1f5f9',color:'#334155',border:'1px solid #cbd5e1',borderRadius:'6px',padding:'8px 16px',fontWeight:600,fontSize:'13px',cursor:busy?'wait':'pointer',opacity:busy?0.7:1,display:'flex',alignItems:'center',gap:'8px'}}>
