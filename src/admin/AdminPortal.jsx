...
function Health(){const [msg,setMsg]=useState('');const [busy,setBusy]=useState(false);const doReset=async()=>{if(!confirm('Clear CSV/imported demo files and assigned adjudications for test scenarios only?\n\nPortal users, sessions, admin fixtures and unrelated records will be left intact.'))return;const reason=prompt('Enter reason for the scoped demo reset (required for audit trail):');if(!reason?.trim())return;setBusy(true);setMsg('');try{const r=await fetch('/api/admin/demo/reset-all',{method:'POST',credentials:'include',headers:{'Content-Type':'application/json'},body:JSON.stringify({reason:reason.trim()})});const body=await r.json();if(!r.ok)throw new Error(body.detail||'Reset failed');const total=Object.values(body.reset||{}).reduce((a,b)=>a+(b||0),0);setMsg(`Scoped reset complete — ${total} CSV/import and assignment records cleared.`)}catch(e){setMsg('Error: '+e.message)}finally{setBusy(false)}};return <Page title="Environment and Health" desc="Platform health, database status and demonstration environment controls."><Notice title="Demonstration environment">All data below is synthetic. Reset does not affect any production system.</Notice><div className="a-grid2"><section className="a-panel"><h2>Database / migration</h2><dl><div><dt>Mode</dt><dd>SQLite (demo) or PostgreSQL (production)</dd></div><div><dt>Auth tables</dt><dd>portal_users · auth_sessions · auth_audit_events</dd></div><div><dt>Longitudinal tables</dt><dd>rt_import_batches · longitudinal_participants · visit_instances</dd></div><div><dt>Adjudication tables</dt><dd>participants · adjudication_records · committee_decisions</dd></div></dl></section><section className="a-panel"><h2>Reset demo CSV scenarios</h2><p style={{fontSize:'13px',color:'var(--text-muted)',marginBottom:'12px'}}>Clears CSV/imported demo batches and their assigned adjudications for repeat test scenarios. Admin accounts, sessions, configuration fixtures and unrelated records are preserved.</p>{msg&&<div className={msg.startsWith('Error')?'a-notice danger':'a-success'} role="status" style={{marginBottom:'12px'}}>{msg}</div>}<button onClick={doReset} disabled={busy} style={{background:'#dc2626',color:'#fff',border:'none',borderRadius:'6px',padding:'10px 20px',fontWeight:700,fontSize:'14px',cursor:busy?'wait':'pointer',opacity:busy?0.7:1,display:'flex',alignItems:'center',gap:'8px'}}><Icon.Trash2 size={15}/>{busy?'Resetting…':'Reset CSV Demo Scenarios'}</button><p style={{fontSize:'11px',color:'var(--text-muted)',marginTop:'8px'}}>Requires admin role · Audited · Scoped to CSV/import scenario data</p></section></div></Page>}
